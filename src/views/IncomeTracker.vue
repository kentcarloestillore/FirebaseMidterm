...
const updateIncome = async () => {
  ...
  try {
    await setDoc(doc(db, 'expenses', incomeId.value), data);
...
<template>
  <div class="adjust justify-center items-center h-full bg-blue-300 shadow-lg rounded-lg p-2">
    <h1 class="title text-4xl font-bold text-center">Income Tracker</h1>
    <div class="container flex flex-col justify-center mx-auto md:flex-row left">
      <div class="left w-[30vh] m-3 p-3 border border-slate-500 rounded h-1/3 bg-gray-200">
        <form @submit.prevent="addIncome">
          <label for="description" class="my-5">Name: </label><br>
          <input type="text"  class="border border-slate-500 rounded text-black mb-5 p-2" id="description" placeholder="Name"><br>
      ...
      <div class="center ml-0 m-3 w-max border border-slate-500 rounded p-3 overflow-auto max-h-[230px]">
        <table class="scrolldown">
          ...
          <tbody>
            <tr v-for="income in incomes">
              <td class="px-3 py-2 border nowrap">{{ income.data.description }}</td>
              <td class="px-3 py-2 border nowrap">{{ "₱" + parseFloat(income.data.amount).toFixed(2) }}</td>
              <td class="px-3 py-2 border nowrap">{{ formatDateTime(income.data.date) }}</td>
              <td class="px-3 py-2 border text-center flex gap-px align-center">
                <button @click="onEdit(income.data, income.id)" class="mx-1 mb-3 mt-3">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-blue-500">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"/>
                  </svg>
                </button>
                <button @click="deleteIncome(income.id)" class="mx-1 mb-3 mt-3">
